...
class WorkerRepository {
  ...
  Future<List<Worker>> fetchWorkers({String? category}) async {
    var query = _client
        .from('workers')
        .select('id, category, experience_years, rating, total_jobs, is_verified, is_online, latitude, longitude, service_radius_km, users!workers_user_id_fkey(name, phone, profile_image)');
  ...
  Future<List<BookingRequest>> fetchMyBookings(String customerId) async {
    final data = await _client
        .from('service_requests')
        .select('*, workers!service_requests_worker_id_fkey(*, users!workers_user_id_fkey(name, profile_image))')
  ...
  Future<List<Proposal>> fetchProposals(int requestId) async {
    final data = await _client
        .from('proposals')
        .select('*, workers!proposals_worker_id_fkey(*, users!workers_user_id_fkey(name, profile_image))')
  ...
  Future<void> respondToProposal(int proposalId, int requestId, String status) async {
